...
class Mode extends Common {
	...
	public function bb() {
		...
		$reg['new_jk'] = db('borrow')->where('uid', $user_id)->order('id desc')->find();
		$reg['tx'] = db('log')->where('user_id', $user_id)->where('type', 50)->select(); //提现记录
		$reg['tb'] = db('log')->where('user_id', $user_id)->where('type', 59)->select(); //提币记录
		$reg['user'] = db('users')->field('user_id,autonym_img,xm,autonym_number,mention_address,nickname,is_autonym,mobile,level,nmct,token,pass,nmct_dj,xmt,money_usdt,token_address,is_red_envelope,bonus,j_bonus,yet_tx_money')->where(array('user_id' => $user_id))->find();
		...
		$arr = explode(',', $reg['user']['autonym_img']);
		$reg['user']['autonym_img_1'] = isset($arr[0]) ? $arr[0] : '';
		$reg['user']['autonym_img_2'] = isset($arr[1]) ? $arr[1] : '';
		$reg['user']['autonym_img_3'] = isset($arr[2]) ? $arr[2] : '';
		unset($reg['user']['autonym_img']);
...
//升级VIP
		...
		if ($type == 8) {
			// $my_money = $number;
			$chack_ = db('borrow')->where('uid', session('user.user_id'))->where('status', 2)->find();
			if (empty($chack_)) {
				return ['status' => 0, 'msg' => '您暂时没有借款！'];
			}
			$msg = Db::name('borrow')
...
//生成激活码
	...
	public function my() {
		$user = db('users')
			->field('user_id,nickname,level,nmct,xmt,g,money_usdt,head_pic,money_cz,mention_address')
