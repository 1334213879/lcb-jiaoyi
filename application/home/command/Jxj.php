	...
	public function jiangli() {
		...
		try {
			...
			foreach ($users as $k => $v) {
				...
				//静态奖
				if ($v['is_overdue'] == 0) {
					$data = ['user_id' => $v['user_id'], 'type' => 57, 'text' => 1500 * 6 / 100 / 30, 'time' => time()];
					Db::name('log')->insert($data);
					db::name('users')->where('user_id', $v['user_id'])->setInc('j_bonus', (1500 * 6 / 100 / 30));
				} else {
					//减少1/3
					$data = ['user_id' => $v['user_id'], 'type' => 57, 'text' => (1500 * 6 / 100) * 2 / 3 / 30, 'time' => time()];
					Db::name('log')->insert($data);
					db::name('users')->where('user_id', $v['user_id'])->setInc('j_bonus', (1500 * 6 / 100) * 2 / 3 / 30);
				}
					...
					if (count($fxid) == 1) {
						...
						if ($v['level'] != 1 && !empty($v['fxid'])) {
							...
							if ($fd['level'] == $v['level']) {
								//平级
								// var_dump(1);
								$data = ['user_id' => $fd['user_id'], 'text' => 1500 * 0.06 * 1 / 100 / 30, 'type' => 55, 'time' => time()];
								Db::name('log')->insert($data);
								db::name('users')->where('user_id', $fd['user_id'])->setInc('bonus', (1500 * 0.06 * 1 / 100 / 30));
								continue;
							} elseif ($v['level'] > $fd['level']) {
								//越级
								// var_dump(2);
								$data = ['user_id' => $fd['user_id'], 'text' => 1500 * 0.06 * 0.5 / 100 / 30, 'type' => 55, 'time' => time()];
								Db::name('log')->insert($data);
								db::name('users')->where('user_id', $fd['user_id'])->setInc('bonus', (1500 * 0.06 * 0.5 / 100 / 30));
								continue;
								...
								$data = ['user_id' => $fd['user_id'], 'text' => 1500 * 6 / 100 * $pl / 30, 'type' => 55, 'time' => time()];
								Db::name('log')->insert($data);
