...
class Users extends Common {
	...
	public function autonym() {
		// ->field('user_id,nickname,is_lock,is_autonym,autonym_img')

		if (request()->isPost()) {
			$key = input('post.key');
			$is_autonym = input('is_autonym');
			$page = input('pageIndex');
			$pageSize = input('pageSize');
			$where = [];
	...
	public function tg() {
		$id = input('id');
		$msg = db('users')->where('user_id', $id)->setField('is_autonym', 1);
		if ($msg) {
			$result['status'] = 1;
			$result['msg'] = '修改成功！';
			return $result;
