...
class Auth extends Common {
	...
	public function goods_order() {
		$xx = db('goods_order')->where(array('status' => 14))->select();
		$this->assign('xx', $xx);
		return $this->fetch('goods_order');
	}
	public function fh() {
		$id = input('id');
		$xx = db('goods_order')->find($id);
		$this->assign('vo', $xx);
		if (request()->isPost()) {
			if (!input('post.kddh') && !input('post.kdgs')) {
				echo "<script>alert('快递单号及公司不能为空');history.go(-1);</script>";die;
			} else {
				db('goods_order')->where('id=' . $id)->update(array('status' => 15));
				db('xx')->where('id=' . $xx['czid'])->update(array('status' => 15, 'kddh' => input('post.kddh'), 'kdgs' => input('post.kdgs'), 'endtime' => time()));
				echo "<script>alert('操作成功');history.go(-2);</script>";die;
			}
		}
		return $this->fetch('fh');
	}
	public function level() {
		$xx = db('xx')->where(array('status' => 10, 'czid' => 0))->select();
		$this->assign('xx', $xx);
		return $this->fetch('level');
	}
	public function level1() {
		$xx = db('xx')->where(array('status' => 10, 'czid' => 1))->order('createtime desc')->select();
		$this->assign('xx', $xx);
		return $this->fetch('level1');
	}
	public function ty() {
		$id = input('post.id');
		if ($id) {
			$xxid = db('xx')->where('id=' . $id)->find();
			$xx = db('xx')->where('id=' . $id)->update(array('czid' => 1));
			db('users')->where('user_id=' . $xxid['userid'])->update(array('level' => $xxid['level']));
		}
		echo "1";
	}
	public function jj() {
